...
@articles_bp.route("/<int:article_id>/comments/<int:comment_id>", methods=["DELETE"])
@jwt_required()
def delete_comment(article_id, comment_id):
    user_id = int(get_jwt_identity())
    comment = Comment.query.filter_by(id=comment_id, article_id=article_id).first_or_404()
    current_user = User.query.get(user_id)
    # Allow if comment owner or admin
    if comment.user_id != user_id and (not current_user or current_user.role != "admin"):
...
@articles_bp.route("/comments/all", methods=["GET"])
@role_required("admin")
